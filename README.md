...
> 如果你想不想API请求量超过免费范围的话，建议`locations.txt`里写入不超过22个`Location_ID`（通过最极端情况计算 50000(免费请求数/月) / 31(1个月的天数) / 72(一个`Location_ID`最多的请求数量/天) ≈ 22 个）
...
2. 运行`main.py`
3. 在PCL里的`设置 - 个性化 - 主页`中选择`联网更新`，填入地址`http://localhost:{port}?location={location}`
   > `{port}`处填入`port.txt`的内容
   > `{location}`处填入选择的`Location_ID`
4. 完成！
...
对应文件`host.txt`，需要你填写访问项目的http网址，协议名（`https://`或`http://`）也需要填写，端口则不用填写，填写格式为`https://example.com`
